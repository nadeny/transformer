...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.params.num_epoch):
            print("=============================== Epoch: ", epoch + 1, " of ", self.params.num_epoch,
                  "===============================")
            self.model.train()
            epoch_loss = 0
            ...
            for batch in self.train_iter:
                print("\rTrain process of epoch: ", epoch + 1, " is running ",
                      format((i / 2875 * 100), ".2f"), "%", end="", flush=True)
            ...
            if valid_loss < best_valid_loss:
                ...
                torch.save(self.model.state_dict(), self.params.save_model, _use_new_zipfile_serialization=False)

            print(f'\rEpoch Time: {epoch_mins}m {epoch_secs}s | Train Loss: {train_loss:.3f} | Val. Loss: {valid_loss:.3f}', end='', flush=True)
            print('\n')
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for batch in self.valid_iter:
                print("\rValidation Process is running ",
                      format((i / 360 * 100), ".2f"), "%", end="", flush=True)
                source = batch.kor
                target = batch.eng
